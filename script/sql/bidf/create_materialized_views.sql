...
DROP MATERIALIZED VIEW VMA_RECETTE;
CREATE MATERIALIZED VIEW VMA_RECETTE REFRESH NEXT SYSDATE + 1/(24) COMPLETE AS
...
LEFT JOIN unite_spec_bud@dblink_elabo_bidf u ON u.usb_code = a.usb_code
LEFT JOIN section_budgetaire@dblink_elabo_bidf s ON s.secb_id = u.secb_id;
ALTER TABLE VMA_RECETTE ADD CONSTRAINT VMA_RECETTE_PK PRIMARY KEY (IDENTIFIANT);
CREATE INDEX VMA_RECETTE_K_EXERCICE ON VMA_RECETTE (EXERCICE ASC);
CREATE INDEX VMA_RECETTE_K_ACTIVITE ON VMA_RECETTE (ACTIVITE_IDENTIFIANT ASC);
CREATE INDEX VMA_RECETTE_K_NE ON VMA_RECETTE (NATURE_ECONOMIQUE_IDENTIFIANT ASC);
CREATE INDEX VMA_RECETTE_K_USB ON VMA_RECETTE (USB_IDENTIFIANT ASC);
CREATE INDEX VMA_RECETTE_K_SECTION ON VMA_RECETTE (SECTION_IDENTIFIANT ASC);