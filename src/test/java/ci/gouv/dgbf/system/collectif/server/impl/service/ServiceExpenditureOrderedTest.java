...
public class ServiceExpenditureOrderedTest {
    ...
    public void get_one_audit() {
		io.restassured.response.Response response = given()
				//.log().all()
				.when().param(ExpenditureService.PARAMETER_NAME_PROJECTIONS, ExpenditureDto.JSON___AUDIT__).get("/api/depenses/1");
		response.then()
		//.log().all()
        	.statusCode(Response.Status.OK.getStatusCode())
        	.body(ExpenditureDto.JSON_IDENTIFIER, equalTo("1"))
