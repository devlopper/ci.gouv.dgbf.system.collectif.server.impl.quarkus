...
public class LegislativeActImplAmountsInitialActualMovementAdjustmentActualPlusAdjustmentReader extends AbstractLegislativeActImplReader implements Serializable {

	@Override
	protected QueryStringBuilder.Arguments instantiateQueryStringBuilderArguments() {
		QueryStringBuilder.Arguments arguments =  super.instantiateQueryStringBuilderArguments();
		arguments.getTuple().addJoins(
				String.format("JOIN %s lav ON lav.%s = t",LegislativeActVersionImpl.ENTITY_NAME,LegislativeActVersionImpl.FIELD_ACT)
				,String.format("LEFT JOIN %s e ON e.%s = lav",ExpenditureImpl.ENTITY_NAME,ExpenditureImpl.FIELD_ACT_VERSION)				
				,String.format("LEFT JOIN %1$s ev ON ev.%2$s = e.%2$s",ExpenditureView.ENTITY_NAME,ExpenditureView.FIELD_IDENTIFIER)
		...
		for(String fieldName : AbstractExpenditureImplReader.ENTRY_AUTHORIZATION_PAYMENT_CREDIT) {
			...
			arguments.getProjection().add(Language.formatSum(FieldHelper.join("t",fieldName,AbstractAmountsImpl.FIELD_ADJUSTMENT))).add(Language.formatSum(FieldHelper.join("available",fieldName)));
		}
