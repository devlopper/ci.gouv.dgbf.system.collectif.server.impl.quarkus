...
public class TransientFieldsProcessorImpl extends org.cyk.utility.persistence.server.TransientFieldsProcessorImpl implements Serializable {
	...
	public void processLegislativeActVersions(Collection<LegislativeActVersionImpl> legislativeActVersions,Collection<String> fieldsNames) {
		for(String fieldName : fieldsNames) {
			...
			else if(LegislativeActVersionImpl.FIELD_ACT_DATE.equals(fieldName))
				new LegislativeActVersionImplLegislativeActDateReader().readThenSet(legislativeActVersions, null);
			...
			else if(LegislativeActVersionImpl.FIELD_IS_DEFAULT_VERSION.equals(fieldName))
	...
	public void processExpenditures(Collection<ExpenditureImpl> expenditures,Collection<String> fieldsNames) {
		for(String fieldName : fieldsNames) {
			...
			else if(ExpenditureImpl.FIELDS_AMOUNTS_WITHOUT_AVAILABLE.equals(fieldName))
				new ExpenditureImplAmountsWithoutAvailableReader().readThenSet(expenditures, null);
			else if(ExpenditureImpl.FIELDS_AMOUNTS_WITHOUT_INCLUDED_MOVEMENT_AND_AVAILABLE.equals(fieldName))
				new ExpenditureImplAmountsWithoutIncludedMovementAndAvailableReader().readThenSet(expenditures, null);
			else if(ExpenditureImpl.FIELDS_AMOUNTS_WITH_INCLUDED_MOVEMENT_ONLY.equals(fieldName))
